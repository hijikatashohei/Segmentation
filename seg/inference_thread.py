...
state_dict = torch.load("./weights/pspnet18_300113_1.pth",
...
def infe():
    # t2 = time.time()
    # 1. 元画像の表示
    image_file_path = "./predata/"+files_pathlist[k]+"/"+pathlist_f[i+1]
    img = Image.open(image_file_path)   # [高さ][幅][色RGB]
...
for k in range(len(files_pathlist)):
    ...
    for i in range(len(pathlist_f)):
        ...
        thread1 = threading.Thread(target=img_pro(y))
        thread2 = threading.Thread(target=infe)
        thread1.start()
        thread2.start()
        # thread1.join()
        # thread2.join()
