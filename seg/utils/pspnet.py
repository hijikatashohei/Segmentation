...
class PSPNet(nn.Module):
    def __init__(self, n_classes):
        ...
        # block_config = [0, 1, 0, 0]
        img_size = 120
        img_size_8 = 15  # img_sizeの1/8に
...
class conv2DBatchNormRelu(nn.Module):  #FeatureMap_convelutionの一層#
...
class FeatureMap_convolution(nn.Module): #FeatureMap_convelutionの全体#
    def __init__(self):
        '''構成するネットワークを用意'''
...
class ResidualBlockPSP(nn.Sequential): #residualBlockPSP#
...
class PyramidPooling(nn.Module): #Pyramid Poolingモジュール#
...
class DecodePSPFeature(nn.Module): #Decoderモジュール#
...
class AuxiliaryPSPlayers(nn.Module): #AuxLossモジュール#
